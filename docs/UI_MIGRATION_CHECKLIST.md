...
The historical phase-one cutoff has been delivered. The current authorized Goal covers the six remaining frontend batches and requires actual review, verified merge and branch cleanup for every delivery. Rules #87, Issue #88, Discussion #89 and Pages #90 are merged with those checks complete. Release/code transfer #91 is merged at `3338a49` after actual review and all final checks; its entire tree equals reviewed `3197c7d` and branch cleanup is complete. Recent repository, Project, Gist and conversation/reaction actions have 1056 passing browser scenarios and 659 passing tests; delivery remains open.
...
| Releases | Lists/details/create/edit and assets | List/detail/create/edit/delete/asset-delete and return/state matrix verified; 264 artifact/code transfer scenarios verified; PR #91 merged and cleaned |
...
| `src/features/github/github-conversation-comment-actions.tsx` | Source reviewed; Issue/PR mutation adapter and reconciliation preserved | Issue400 and Discussion248 scenarios cover comment/reply recovery, pending guards and permissions; PR #88/#89 merged and cleaned. |
| `src/features/github/github-conversation-controls.tsx` | Retained-query notice, 11 px label; recovery regression passes | Conversation controls:192 write +80 read scenarios verified; PR #92 awaits actual review. |
...
| `src/features/github/github-gist-detail.tsx` | Independent comment drafts, scoped errors/pending controls and guarded delete confirmations | 224 Gist action/read scenarios and shared landmark matrix verified; PR #92/shared delivery pending. |
| `src/features/github/github-gist-editor-dialog.tsx` | Pending dismissal/input guard, refresh-safe drafts, named visibility group | 224 Gist action/read scenarios verify drafts, guards, duplicate files and recovery; PR #92 awaits actual review. |
| `src/features/github/github-gist-view.tsx` | Audited and migrated in Notifications/More batch | 224 Gist scenarios and32shared landmark combinations across four pages verified; delivery pending. |
...
| `src/features/github/github-issue-state-action.tsx` | Audited; shared split action and Radix menu retained | 400 Issue scenarios include close/reopen, pending/error and retained drafts; PR #88 merged and cleaned. |
...
| `src/features/github/github-project-dialogs.tsx` | Pending guards, draft retention, read-only and empty-choice states implemented | 240 Project scenarios verify field/draft/settings/item recovery, permissions and empty choices; PR #92 awaits actual review. |
...
| `src/features/github/github-project-view.tsx` | Permissions reach field/item/draft actions; pending archive/delete guards and wrapping text | 240 Project scenarios plus shared main-landmark verification; delivery pending. |
...
| `src/features/github/github-reaction-bar.tsx` | Compact stale Retry, retained empty read-only feedback, keyboard description; recovery regressions pass | 96 reaction write +48 read scenarios verify pending, rollback, readonly owned removal, stale empty readonly and keyboard menu behavior; PR #92 awaits actual review. |
| `src/features/github/github-reactions-provider.tsx` | Source reviewed; exposes refresh state; batching, optimistic updates and rollback preserved | 96 reaction write +48 read scenarios verify query/mutation reconciliation and retained failures; PR #92 awaits actual review. |
...
| `src/features/github/github-repository-create-dialog.tsx` | Linked labels, pending/dismissal guards, inline/resettable failure and retained optional templates | 24 create +24 optional-template scenarios verify pending/drafts and optional read recovery; PR #92 awaits actual review. |
...
| `src/features/github/github-repository-pages-view.tsx` | Audited; shared surfaces, readable metadata and retained-data feedback applied where needed | 224 archived/Pages scenarios verify conditions, writes, retry, health width and metadata return; PR #90 merged and cleaned. |
| `src/features/github/github-repository-relationship-actions.tsx` | Shared controls/stale feedback, Fork guards, repository identity isolation, failed Star count recovery | 40 fork +72 relationship +16 switch/return captures verified; PR #92 awaits actual review. |
| `src/features/github/github-repository-settings-view.tsx` | Audited; shared surfaces, readable metadata and retained-data feedback applied where needed | Historical settings matrix plus224 archived/Pages scenarios verified; PR #90 merged and cleaned. |
