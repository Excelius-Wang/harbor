...
- `pnpm check` passes: 751 tests in 145 files, formatting, lint, TypeScript and production build. Log: `/tmp/harbor-encounters-check-final.log`. Existing harbor-rail ref-cleanup lint warning, Node SQLite experimental warning and bundle-size warnings remain.
...
- [State results](state-results.json): reused from the preceding revision for unchanged query/loading/error/stale handling only; these are not fresh rooftop screenshots. The 32 combinations cover loading, retryable error, zero-contribution data and stale retained results across both languages/themes/widths at height 760. Stale selection and display switching remain usable.
...
- [Prior motion results](motion-results.json), reused for unchanged travel classification and annual foot positioning: nearby hop, distant fade, rapid replacement, acknowledged background pause, resize and reduced motion pass. Feet and contact-shadow centers remain within 1 px of the selected cap.
...
Run `pnpm dev:ui --port 1439`. Open `/ui-components?view=opportunities&calendar=real&links=record` in Playwright CLI, then run [encounters.js](encounters.js) and [encounter-actions.js](encounter-actions.js). The prior [qa.js](qa.js), [motion.js](motion.js), [states.js](states.js) and [dense.js](dense.js) retain historical evidence and assumptions; do not use the old no-encounter assertion for the current UI. Run current scripts sequentially with `run-code --filename`. Create `output/playwright/calendar-3d/` first. All business calls remain intercepted.
