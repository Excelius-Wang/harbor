async (page) => {
  ...
  const idle = () => page.waitForFunction(() => {
    ...
    return marker && getComputedStyle(marker).visibility === 'visible' && marker.getAnimations().every(a => a.playState === 'finished' || a.playState === 'idle');
  });
      ...
      for (const width of [900, 1440]) {
        ...
        const check = async () =>
          page.evaluate(() => {
            ...
            return {
              footX: Math.abs(sprite.left + sprite.width / 2 - face.left - face.width / 2),
              ...
              noEncounters: !document.querySelector('.harbor-calendar-companion .harbor-companion-object'),
        ...
        if (annual.footX > 1 || annual.footY > 1 || !annual.continuous || !annual.noEncounters || !annual.bounded || annual.overflow)
          throw Error(JSON.stringify({ key, annual }));
        await calendar.scrollIntoViewIfNeeded();
        await page.screenshot({ path: `output/playwright/calendar-3d/rooftop-${key}-year.png` });
        await page
        ...
        if (monthly.footX > 1 || monthly.footY > 1 || !monthly.continuous || !monthly.noEncounters || !monthly.bounded || monthly.overflow)
          throw Error(JSON.stringify({ key, monthly }));
        await calendar.scrollIntoViewIfNeeded();
        await page.screenshot({ path: `output/playwright/calendar-3d/rooftop-${key}-month.png` });
        await page
          .getByRole("button", {
