...
## 3D contribution calendar — 2026-09-25

`ContributionCalendar` defaults to a fixed oblique 3D appearance with an independent Flat/3D radio group. Column extrusion is linearly normalized to the annual maximum and capped at 44 px above a common 4 px base; month view uses the same scale. Semantic contribution colors, native date buttons, roving focus and Radix tooltips remain in use. `CalendarCompanion.onColumn` uses `CalendarColumnResident` to anchor feet and a contact shadow at the measured selected top-face center. Year/month sprites are 18/24 px, without reserved lanes. Nearby dates hop; distant dates fade. Selection replaces movement, pause/visibility suspends it, and reduced motion places directly. Resize and data refresh remeasure without replay. 3D mode restores chest/bug encounters: annual marks indicate events, monthly objects share the cap with the resident at 32% and the object at 77% of its width. Arrival/action/feedback each last 180 ms. Completed dates count once per session, including zero-contribution dates, without changing experience; reduced motion shows static results. Flat behavior remains unchanged. Tooltips are transient and offset above raised tops. Selected-date details are below the year grid, beside the month grid at container widths of at least 820 px, and below it otherwise. Loading uses a taller placeholder; refresh and empty/error states preserve existing query handling. [Scoped verification](verification/calendar-3d/README.md).
