...
App 缓存保留最近 90 天更新的最多 10,000 条 issue 记录，超出时清理最旧记录；移除仓库后清理该仓库的记录和检查进度。只保存推荐摘要和去重摘要，不持久化原始 issue 正文。过期记录若早于首次监控基线，重新打开后可能需要历史试筛才会重新纳入。损坏的序列化状态会备份到数据库中的单条恢复记录，保留可验证的设置、重置运行状态并显示提示；真实数据库读写错误仍会报错。
