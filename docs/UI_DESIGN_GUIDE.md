...
首次打开显示 226 px 的完整导航，点击标题栏的产品 Logo 可将其收起为 58 px 图标栏，不再增加独立的侧栏按钮。Logo 外观保持不变，通过悬停提示和焦点反馈说明操作。记住用户的选择，调整窗口宽度时不自动覆盖。1280 px 断点继续控制内容区布局，不再决定主导航是否显示文字。收起时保留各入口的名称提示、选中态和键盘焦点。
