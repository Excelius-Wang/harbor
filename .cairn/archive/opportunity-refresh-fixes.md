...
- Local implementation complete on `fix/opportunity-refresh-state` in the primary Harbor worktree, base `8cb6765`.
...
- Independent notification batch is in the notification worktree on `fix/notification-inflight`.
...
- 27 native monitor tests and cargo check pass. Log `/tmp/harbor-opportunity-refresh-native.log`. Red phase reproduced three failures before the fix.
