...
describe("GitHub Issue mutations", () => {
  ...
  it("invalidates assigned-to-me caches when an Issue changes", () => {
    const queryClient = new QueryClient();
    const assignedToMeListKey = githubQueryKeys.issues({
      owner: target.owner,
      repository: target.repository,
      state: "open",
    ...
    queryClient.setQueryData<GitHubIssuePage>(assignedToMeListKey, {
      issues: [issue],
      totalCount: 1,
      page: 1,
      hasPrevious: false,
    ...
    syncUpdatedIssue(queryClient, target, {
      ...issue,
      assignees: ["another-user"],
    });

    expect(queryClient.getQueryData<GitHubIssuePage>(assignedToMeListKey)?.issues).toEqual([issue]);
    expect(queryClient.getQueryState(assignedToMeListKey)?.isInvalidated).toBe(true);
  });

  it("invalidates created-by-me caches when an Issue changes", () => {
    const queryClient = new QueryClient();
    const createdByMeListKey = githubQueryKeys.issues({
      owner: target.owner,
      repository: target.repository,
      state: "open",
      assignment: "all",
    ...
    queryClient.setQueryData<GitHubIssuePage>(createdByMeListKey, {
      issues: [issue],
      totalCount: 1,
      page: 1,
      hasPrevious: false,
    ...
    expect(queryClient.getQueryData<GitHubIssuePage>(createdByMeListKey)?.issues).toEqual([issue]);
